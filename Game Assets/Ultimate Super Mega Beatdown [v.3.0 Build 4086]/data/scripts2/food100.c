void main()
{//Script for food with 100 health
    ...
    if (Health >= MHealth){
      changeplayerproperty(PIndex, "score", PScore+10000); // Give score to player if his/her health is full
    } else {
      changeentityproperty(target, "health", Health+99); // Give health instead if health is not full
    }
}