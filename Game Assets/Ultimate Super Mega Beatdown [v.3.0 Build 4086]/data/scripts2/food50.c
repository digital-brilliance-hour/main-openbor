void main()
{//Script for food with 50 health
    ...
    if (Health >= MHealth){
      changeplayerproperty(PIndex, "score", PScore+5000); // Give score to player if his/her health is full
    } else {
      changeentityproperty(target, "health", Health+49); // Give health instead if health is not full
    }
}