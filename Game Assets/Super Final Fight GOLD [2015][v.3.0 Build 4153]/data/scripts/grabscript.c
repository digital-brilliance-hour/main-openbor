...
// Mainly used for slams
   ...
   if(target!=NULL())
   {
     ...
     if(THealth > Damage)
     {
       damageentity(target, self, Damage, 1, openborconstant("ATK_NORMAL5")); // Damage target with desired damage
       updateframe(target, TAniPos);
     } else {
       int Damage2 = THealth - 1;
       damageentity(target, self, Damage2, 1, openborconstant("ATK_NORMAL5")); //Damage target with less damage
       updateframe(target, TAniPos);
     }
   }
}
...
{ // Damage as slam or throw finisher
   ...
   if(target!=NULL())
   {
     ...
     if(Type==2)
     {
       damageentity(target, self, Damage, 1, openborconstant("ATK_NORMAL5")); // 2nd Finisher
     }
