...
//Double forwardjump
    if(vAniID == openborconstant("ANI_FORWARDJUMP") ){ //Jumping?
      if(iJump){ //Left pressed?
        changeentityproperty(vSelf, "animation", openborconstant("ANI_FREESPECIAL2"));
        if(iLeftH){ //Left held?
          changeentityproperty(vSelf, "animation", "direction", 0);
          tossentity(vSelf, 2.3, -1.3, 0);
        } else if(iRightH){ //Right held?
          changeentityproperty(vSelf, "animation", "direction", 1);
          tossentity(vSelf, 2.3, 1.3, 0);
        } else { //Nothing
          tossentity(vSelf, 2.3, 0, 0);
        }
      }
    }
...
//Double jump
    if(vAniID == openborconstant("ANI_JUMP") ){ //Jumping?
      if(iJump){ //Left pressed?
        changeentityproperty(vSelf, "animation", openborconstant("ANI_FREESPECIAL1"));
        if(iLeftH){ //Left held?
          changeentityproperty(vSelf, "animation", "direction", 0);
          tossentity(vSelf, 2.3, -1.3, 0);
        } else if(iRightH){ //Right held?
...
//Mid-air attack
    if(vAniID == openborconstant("ANI_JUMP") || vAniID == openborconstant("ANI_FORWARDJUMP") ){ //Jumping?
      if (iAttack){ //Attack pressed?
        changeentityproperty(vSelf, "animation", openborconstant("ANI_JUMPATTACK") || vSelf, "animation", openborconstant("ANI_FREESPECIAL3") || vSelf, "animation", openborconstant("ANI_FREESPECIAL4"));
      }
    }
