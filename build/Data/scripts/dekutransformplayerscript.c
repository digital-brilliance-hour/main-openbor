...
void main()
{
   ...
   if(AniID != openborconstant("ANI_FALL")) {
      void pbind = getlocalvar("bolt_bind");
      if(pbind == NULL()) {
         void bbolts = spawnbind("deku_bluebolts", 0, 0, -20);
         setlocalvar("bolt_bind", bbolts);
      }
   }
   else {
      void pbind = getlocalvar("bolt_bind");
